...
class ConsultaCpeService
{
    ...
    {
        // La Consulta de Validez de SUNAT solo opera en PRODUCCIÓN (documentos
        // reales). En beta no existe este servicio, por eso se avisa claramente.
        if (! $this->company->modo_produccion) {
            return [
                'success' => false,
                'message' => 'La Consulta CPE solo funciona en PRODUCCIÓN, con documentos y credenciales reales. En modo beta (pruebas) SUNAT no ofrece este servicio.',
                'data' => null,
            ];
        }

        try {
            // Obtener token válido. La API OAuth2 de Consulta CPE requiere
            // credenciales con permiso de consulta (distintas a las de GRE). Si
            // no hay token, se usa la consulta tradicional por SOAP (clave SOL).
    ...
    {
        // Solo se registra el resultado de la consulta; NO se sobrescribe el
        // estado_sunat de la emisión (la consulta es de verificación).
        $documento->update([
            'consulta_cpe_estado' => $estados['descripcion_estado'] ?? ($estados['estadocpe'] ?? null),
    ...
    /**
     * Credenciales SOL de la empresa para la consulta SOAP.
     */
    ...
    /**
     * Interpreta el resultado SOAP (getStatus) a un arreglo de estados.
     * Códigos SUNAT: 0001 aceptado · 0002 no existe · 0003 baja · 0004 anulado.
     */
    protected function procesarEstadosSoap($result): array
    {
        ...
        return [
            'estadocpe' => $code,
            'descripcion_estado' => $map[$code] ?? ($result->getMessage() ?: 'Estado desconocido'),
            'mensaje_sunat' => $result->getMessage(),
            'consulta_fecha' => date('Y-m-d H:i:s'),
        ];
    }

    /**
     * Guarda el CDR (zip) devuelto por la consulta.
     */
    protected function guardarCdr($documento, string $cdrZip): ?string
    {
        try {
            return app(FileService::class)->saveCdr($documento, $cdrZip);
        } catch (\Throwable $e) {
            Log::warning('No se pudo guardar CDR de consulta: ' . $e->getMessage());
            return null;
        }
    }
