...
class DocumentService
{
    ...
    public function sendToSunat($document, string $documentType): array
    {
        try {
            ...
            // Obtener hash del XML (no-crítico, puede fallar sin problema)
            if ($result['success'] && $result['cdr_zip']) {
                try {
                    ...
                    \Illuminate\Support\Facades\Log::warning('Error extracting hash: ' . $e->getMessage());
                }
            }
    ...
    protected function calculateTotals(array &$detalles, array $globalData = []): array
    {
        ...
        foreach ($detalles as &$detalle) {
            ...
            $precioUnitarioIncluyeIgv = $detalle['precio_unitario_incluye_igv'] ?? null;
            ...
            if ($tipAfeIgv === '10') { // Gravado - paga IGV
                if ($precioUnitarioIncluyeIgv !== null && $porcentajeIgv > 0 && empty($detalle['descuentos'])) {
                    $totalConIgv = round($cantidad * (float) $precioUnitarioIncluyeIgv, 2);
                    $mtoBaseIgv = round($totalConIgv / (1 + ($porcentajeIgv / 100)), 2);
                    $mtoValorVenta = $mtoBaseIgv;
                    $igv = round($totalConIgv - $mtoBaseIgv, 2);
                } else {
    ...
    /**
     * Genera la leyenda 1000 (monto en letras) del XML enviado a SUNAT.
     * Reutiliza la conversión completa ya existente (convertirNumeroALetras).
     */
    ...
    public function createSummaryFromBoletas(array $data): DailySummary
    {
        return DB::transaction(function () use ($data) {
            ...
            // Se cargan los clientes con eager loading (with) para evitar una
            // consulta extra por cada boleta al construir los detalles (N+1).
            $boletas = Boleta::with('client:id,tipo_documento,numero_documento')
            ...
            // Vincular boletas al resumen con un solo UPDATE masivo
            // en lugar de una consulta por boleta.
            Boleta::whereIn('id', $boletas->pluck('id'))
                ->update(['daily_summary_id' => $summary->id]);
    ...
    public function sendDispatchGuideToSunat(DispatchGuide $guide): array
    {
        try {
            ...
            // Endpoint GRE segun el ambiente de la empresa: el sistema cambia
            // automaticamente entre BETA (pruebas) y PRODUCCION segun modo_produccion.
            // Las credenciales GRE tambien cambian solas (getGreClientId, getGreClaveSol).
            $company = $guide->company;

            // Endpoints segun el ambiente (sandbox/beta/produccion).
            $endpoints = $company->greApiEndpoints();
            $api = new \Greenter\Api($endpoints);

            Log::info('Endpoint GRE seleccionado segun ambiente', [
                'ambiente' => $company->greAmbienteLabel(),
                'cpe' => $endpoints['cpe'],
            ]);

            // Certificado en formato PEM. Si la empresa subio su .pfx/.p12 se
            // convierte aqui (con su contrasena); si no, se cae al compartido.
            $certificadoContent = $this->getCompanyCertificatePem($company);
            
            // Validar credenciales GRE de la empresa (ya cambian solas segun el ambiente)
    ...
    /**
     * Devuelve el certificado digital de la empresa en formato PEM (lo que
     * necesita Greenter). Si la empresa subio un .pfx/.p12, lo convierte aqui
     * usando su contrasena. Si no tiene certificado propio, usa el compartido
     * (solo valido para desarrollo/pruebas).
     */
    protected function getCompanyCertificatePem(Company $company): string
    {
        $stored = $company->certificado_pem;

        // Caso 1: la empresa subio su certificado .pfx/.p12 (se guarda la ruta en disco 'local')
        ...
        if (!empty($stored) && str_contains($stored, '-----BEGIN')) {
            return $stored;
        }

        // En PRODUCCION jamas se firma con el certificado de prueba compartido:
        // SUNAT lo rechazaria y serian documentos reales. Se exige el propio.
        if ($company->modo_produccion) {
            throw new Exception('La empresa está en PRODUCCIÓN pero no tiene un certificado digital propio cargado. Sube tu certificado real (.pfx) en Configuración SUNAT antes de emitir.');
        }
        ...
        throw new Exception("La empresa {$company->razon_social} no tiene un certificado digital configurado.");
    }

    /**
     * Prueba la conexion con SUNAT pidiendo un token OAuth con las credenciales
     * de la empresa, SIN enviar ningun documento. Sirve para saber si las
     * credenciales son validas antes de emitir. El ambiente (beta/produccion)
     * lo determina modo_produccion.
     *
     * @return array{success: bool, message: string}
     */
    public function testSunatConnection(Company $company): array
    {
        try {
            $clientId = $company->getGreClientId();
            $clientSecret = $company->getGreClientSecret();
            $ruc = $company->getGreRucProveedor();
            $usuario = $company->getGreUsuarioSol();
            $clave = $company->getGreClaveSol();
            ...
            if (empty($clientId) || empty($clientSecret)) {
                return [
                    'success' => false,
                    'message' => "Faltan el Client ID / Client Secret ({$ambiente}). Configuralos primero.",
                ];
            }
            ...
            if (empty($ruc) || empty($usuario) || empty($clave)) {
                return [
                    'success' => false,
                    'message' => 'Faltan RUC proveedor / Usuario SOL / Clave SOL para GRE.',
                ];
            }

            // Flujo OAuth real de GRE: grant "password" con el usuario SOL.
            // El endpoint de seguridad (auth) y el scope (cpe) cambian segun el
            // ambiente: sandbox (nubefact), beta o produccion (SUNAT).
            $endpoints = $company->greApiEndpoints();
            $scope = rtrim($endpoints['cpe'], '/');
            $tokenUrl = rtrim($endpoints['auth'], '/') . "/clientessol/{$clientId}/oauth2/token/";
            ...
            if ($status === 200 && !empty($body['access_token'])) {
                return [
                    'success' => true,
                    'message' => 'Conexion exitosa. Credenciales GRE validas.',
                ];
            }

            $error = $body['error_description'] ?? $body['error'] ?? ('HTTP ' . $status);

            return [
                'success' => false,
                'message' => "El servidor rechazo las credenciales: {$error}",
            ];

        } catch (\Throwable $e) {
            Log::warning('Fallo prueba de conexion SUNAT', [
                'company_id' => $company->id,
                'error' => $e->getMessage(),
            ]);
    ...
    public function checkDispatchGuideStatus(DispatchGuide $guide): array
    {
        try {
            ...
            // Misma configuracion que el envio: endpoints + credenciales + certificado
            // de la EMPRESA (cambian solos entre beta/produccion segun modo_produccion).
            $company = $guide->company;
    ...
    public function createVoidedDocument(array $data): VoidedDocument
    {
        return DB::transaction(function () use ($data) {
            ...
            // Crear comunicación de baja (columnas reales de voided_documents)
            $voidedDocument = VoidedDocument::create([
                'company_id' => $company->id,
                'branch_id' => $branch->id,
                'correlativo' => $correlativo,
                'fecha_generacion' => now()->toDateString(),      // Fecha de comunicación (hoy)
                'fecha_referencia' => $data['fecha_referencia'],  // Fecha de documentos a anular
                'motivo' => $data['motivo_baja'] ?? $data['motivo'] ?? 'Anulación de la operación',
                'detalles' => $data['detalles'],
                'estado_sunat' => 'PENDIENTE',
                'usuario_creacion' => $data['usuario_creacion'] ?? null,
            ]);
    ...
    protected function prepareVoidedDocumentData(VoidedDocument $voidedDocument): array
    {
        return [
            ...
            'fecha_emision' => $voidedDocument->fecha_generacion->toDateString(),   // fecha de comunicación
            'fecha_referencia' => $voidedDocument->fecha_referencia->toDateString(),
            'detalles' => $voidedDocument->detalles,
            'motivo_baja' => $voidedDocument->motivo,
        ];
    }
    ...
    public function convertirNumeroALetras($numero, $moneda = 'PEN'): string
    {
        ...
        // mb_strtoupper para respetar tildes (Ó, É): "millón" -> "MILLÓN".
        return mb_strtoupper($resultado . ' ' . $monedaNombre, 'UTF-8');
    }
    ...
    protected function convertirEntero($numero, $unidades, $decenas, $centenas): string
    {
        ...
        // 21-29 se escriben unidos: veintiuno, veintidós, ... (forma correcta).
        if ($numero < 30) {
            $veintis = [
        ...
        if ($numero < 1000000000) {
            ...
            if ($millones == 1) {
                ...
                $resultado = $this->convertirEntero($millones, $unidades, $decenas, $centenas) . ' millones';
            }
