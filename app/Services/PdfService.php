...
class PdfService
{
    ...
    protected function prepareDispatchGuideData($dispatchGuide): array
    {
        return [
            ...
            'destinatario' => $dispatchGuide->client,
    ...
    protected function generateQRData($document): string
    {
        ...
        // Valor resumen (DigestValue de la firma) = último campo obligatorio del QR SUNAT.
        $hash = $document->codigo_hash ?? '';

        return "{$ruc}|{$tipoDoc}|{$serie}|{$numero}|{$mtoIgv}|{$mtoTotal}|{$fechaEmision}|{$tipoDocCliente}|{$numDocCliente}|{$hash}|";
    }
