...
class StoreInvoiceRequest extends FormRequest
{
    ...
    {
        $client = $this->normalizeClientPayload($this->input('client', $this->input('cliente', [])));
        $details = $this->normalizeDetailsPayload($this->input('detalles', $this->input('items', [])));
        $cuotas = $this->input('forma_pago_cuotas', $this->input('cuotas'));

        if (is_array($cuotas)) {
            $cuotas = array_map(fn ($cuota) => [
                'moneda' => $cuota['moneda'] ?? $this->input('tipo_moneda', 'PEN'),
                'monto' => $cuota['monto'] ?? 0,
        ...
        $this->merge([
            'client' => $client,
            'detalles' => $details,
            'moneda' => $this->input('moneda', $this->input('tipo_moneda', 'PEN')),
            'forma_pago_tipo' => $this->input('forma_pago_tipo', $this->input('forma_pago', 'Contado')),
            'forma_pago_cuotas' => $cuotas,
            'datos_adicionales' => $this->input('datos_adicionales', [
