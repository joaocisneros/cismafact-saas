...
class ConsultaController extends Controller
{
    public function index()
    {
        return view('super-admin.consultas.index', [
            ...
            'apis' => Api::with(['planes' => fn ($q) => $q->orderBy('a_medida')->orderBy('precio_mensual')->orderBy('orden')])
                ...
                ->get(),
            'planesApi' => ApiPlan::with('apis')->orderBy('a_medida')->orderBy('precio_mensual')->orderBy('orden')->get(),
            /*
            ...
            // Quien puede entrar a ver su llave. Solo las de produccion: las de
            // Sandbox son para probar y no dan acceso al sistema.
            'titulares' => $this->titularesConAcceso(),
    ...
    public function guardarPlan(Request $request)
    {
        $datos = $request->validate([
            ...
            'precios' => 'array',
            'precios.*' => 'numeric|min:0|max:99999',
        ]);
        ...
        $plan = ApiPlan::create([
            ...
            'precio_mensual' => $this->sumaDeLosServicios($precios),
        ...
        foreach (Api::pluck('id') as $api) {
            $plan->apis()->attach($api, [
                'limite_mensual' => max(0, (int) ($cuotas[$api] ?? 0)),
                'precio_mensual' => max(0, (float) ($precios[$api] ?? 0)),
            ]);
        }
    ...
    public function actualizarPlan(Request $request, ApiPlan $plan)
    {
        ...
        // El precio del plan es lo que suman sus servicios, no un numero
        // aparte: si no, se podia dejar el plan en S/39 con los servicios a
        // cero y cobrar una cosa distinta de la que se ensena.
        $datos['precio_mensual'] = $this->sumaDeLosServicios($precios);
        unset($datos['precios']);
        ...
        foreach ((array) $request->input('cuotas', []) as $api => $tope) {
            $plan->apis()->syncWithoutDetaching([$api => [
                'limite_mensual' => max(0, (int) $tope),
                'precio_mensual' => max(0, (float) ($precios[$api] ?? 0)),
            ]]);
        }
    ...
    /**
     * Lo que cuesta el plan: la suma de sus servicios.
     *
     * El precio dejo de ser del plan y paso a ser de cada consulta, porque el
     * RUC es publico y lo da la propia SUNAT mientras que el de RENIEC hay que
     * conseguirlo. Quien contrata uno solo paga solo ese.
     */
    private function sumaDeLosServicios(array $precios): float
    {
        return round(array_sum(array_map(
            fn ($precio) => max(0, (float) $precio),
            $precios,
        )), 2);
    }
    ...
    private function titularesConAcceso()
    {
        return \App\Models\ConsultaLlave::where('entorno', 'produccion')
            ...
            ->map(fn ($llaves) => [
                'usuario' => $llaves->first()->usuario,
                'titular' => $llaves->first()->titular ?: $llaves->first()->nombre,
                'correo' => $llaves->first()->titular_email,
                'llaves' => $llaves,
            ])
            ->values();
    }
