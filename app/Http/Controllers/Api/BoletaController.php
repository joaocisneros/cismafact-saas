...
class BoletaController extends Controller
{
    ...
    public function store(StoreBoletaRequest $request): JsonResponse
    {
        try {
            ...
            if (($validated['metodo_envio'] ?? 'individual') === 'individual') {
                $sendResult = $this->documentService->sendToSunat($boleta, 'boleta');
                $boleta = $sendResult['document'];

                return response()->json([
                    'success' => $sendResult['success'],
                    'data' => $boleta->load(['company', 'branch', 'client']),
                    'message' => $sendResult['success']
                        ? 'Boleta creada y enviada a SUNAT correctamente'
                        : 'Boleta creada, pero SUNAT respondio con error',
            ...
            return response()->json([
                'success' => true,
                'data' => $boleta->load(['company', 'branch', 'client']),
                'message' => 'Boleta creada correctamente. Queda pendiente para resumen diario.'
            ], 201);
