...
<div class="space-y-5"
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Nueva Boleta</h1>
            <p class="text-gray-500 mt-1">Completa los datos y emite directo a SUNAT.</p>
        </div>
        <a href="{{ route('empresa.boletas.index') }}" class="text-sm text-gray-600 hover:text-gray-900">← Volver</a>
    </div>
    ...
    <form method="POST" action="{{ route('empresa.boletas.store') }}" class="space-y-5">
        @csrf
        ...
        <div class="flex justify-end gap-2">
            <a href="{{ route('empresa.boletas.index') }}" class="rounded-md border border-gray-300 px-5 py-2.5 text-sm">Cancelar</a>
...
function boletaForm(config) {
    return {
        ...
        get totales() {
            ...
            return { gravadas, exoneradas, inafectas, exportacion, igv, total: gravadas + exoneradas + inafectas + exportacion + igv };
        },
        ...
        agregarItem() { this.items.push({ codigo: String(this.items.length + 1).padStart(3, '0'), descripcion: '', unidad: 'NIU', cantidad: 1, valorUnitario: 0, afectacion: '10' }); },
