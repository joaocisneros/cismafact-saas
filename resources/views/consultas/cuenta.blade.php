...
<div class="mb-5">
    ...
    <p class="text-sm text-gray-600">Tus datos para entrar aquí. No tienen nada que ver con la API.</p>
</div>

<div class="grid gap-4 lg:grid-cols-2">
    <div class="rounded-xl border border-gray-200 bg-white">
        <div class="border-b border-gray-100 px-4 py-3">
            <h2 class="text-[11px] font-semibold uppercase tracking-widest text-gray-400">Datos</h2>
        </div>
        <form method="POST" action="{{ route('consultas.cuenta.guardar') }}" class="space-y-3 p-4">
            @csrf
            @method('PUT')
            ...
            <label class="block">
                <span class="mb-1 block text-xs font-semibold text-gray-600">Correo — con este entras</span>
    ...
    <div class="rounded-xl border border-gray-200 bg-white">
        <div class="border-b border-gray-100 px-4 py-3">
            <h2 class="text-[11px] font-semibold uppercase tracking-widest text-gray-400">Contraseña</h2>
        </div>
        <form method="POST" action="{{ route('consultas.cuenta.clave') }}" class="space-y-3 p-4">
            @csrf
            @method('PUT')

            <label class="block">
                <span class="mb-1 block text-xs font-semibold text-gray-600">Contraseña actual</span>
                <input name="actual" type="password" required autocomplete="current-password"
                       class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm">
            </label>

            <label class="block">
                <span class="mb-1 block text-xs font-semibold text-gray-600">Contraseña nueva</span>
            ...
            <label class="block">
                <span class="mb-1 block text-xs font-semibold text-gray-600">Repite la nueva</span>
