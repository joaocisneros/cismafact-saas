            ...
            <div class="flex items-center gap-2">
                ...
                <div x-data="{ menu: false }" @click.outside="menu = false" class="relative">
                    <button type="button" @click="menu = ! menu"
                            title="Más opciones"
                            class="grid h-8 w-8 place-items-center rounded-md text-gray-400 hover:bg-gray-100 hover:text-gray-700">
                    ...
                    <div x-show="menu" x-cloak x-transition.opacity
                        ...
                        <form method="POST" action="{{ route('consultas.secreto.regenerar', $llave->id) }}"
                            ...
                            <button type="submit" :disabled="enviando"
                                    class="flex w-full items-start gap-2.5 px-4 py-2.5 text-left text-sm text-red-700 hover:bg-red-50 disabled:opacity-60">
                                <svg class="mt-0.5 h-4 w-4 shrink-0 text-red-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M21 12a9 9 0 1 1-3-6.7M21 3v6h-6"/>
                                </svg>
                                <span>
                                    <span class="block font-semibold" x-text="enviando ? 'Generando…' : 'Generar un secreto nuevo'">Generar un secreto nuevo</span>
                                    <span class="block text-xs text-gray-500">Solo si el tuyo se filtró. El actual deja de valer.</span>
                                </span>
                            </button>
                        </form>
                    </div>
