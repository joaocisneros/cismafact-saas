...
@section('content')
<div class="mb-5">
    <h1 class="text-xl font-semibold text-gray-900">Mi API</h1>
    <p class="text-sm text-gray-600">Estas dos cosas van en cada llamada que hace tu programa.</p>
</div>
...
@forelse($llaves as $llave)
    <div class="mb-4 rounded-xl border border-gray-200 bg-white"
        ...
        <div class="flex flex-wrap items-center justify-between gap-2 border-b border-gray-100 px-4 py-3">
            <div class="min-w-0">
                <h2 class="truncate text-sm font-semibold text-gray-900">{{ $llave->nombre }}</h2>
        ...
        <div class="space-y-4 p-4">
            <div>
                ...
                <div class="flex gap-2">
                    <code class="flex-1 overflow-x-auto whitespace-nowrap rounded-md border border-gray-200 bg-gray-50 px-3 py-2 font-mono text-xs">{{ $llave->clave }}</code>
                    <button type="button"
                            @click="navigator.clipboard.writeText(clave); $el.textContent = 'Copiada'; setTimeout(() => $el.textContent = 'Copiar', 1400)"
                            class="shrink-0 rounded-md border border-gray-300 px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50">
                        Copiar
                    </button>
                </div>
            </div>
            ...
            <div>
                ...
                <div class="flex gap-2">
                    <code class="flex-1 overflow-x-auto whitespace-nowrap rounded-md border border-gray-200 bg-gray-50 px-3 py-2 font-mono text-xs"
            ...
            <dl class="grid grid-cols-2 gap-4 border-t border-gray-100 pt-4 sm:grid-cols-4">
                <div>
                    ...
                    <dd class="text-sm font-semibold text-gray-900">{{ $llave->plan?->nombre ?? '—' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500">Servicios</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ collect($llave->servicios)->map(fn ($s) => strtoupper($s))->join(', ') }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500">Entorno</dt>
                    <dd class="text-sm font-semibold text-gray-900">{{ $llave->entorno === 'produccion' ? 'Producción' : 'Pruebas' }}</dd>
                </div>
                <div>
                    <dt class="text-xs text-gray-500">Vigencia</dt>
                    <dd class="text-sm font-semibold tabular-nums text-gray-900">{{ $llave->expira_en?->format('d/m/Y') ?? 'sin caducidad' }}</dd>
                </div>
            </dl>
