...
<div class="mb-5 flex flex-wrap items-start justify-between gap-3">
    <div>
        <h1 class="text-xl font-semibold text-gray-900">Panel</h1>
        <p class="text-sm text-gray-600">Tu servicio de consultas, de un vistazo.</p>
    </div>
...
@else
    <div class="mb-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <div class="rounded-xl border border-gray-200 bg-white p-4">
            ...
            <p class="text-xs text-gray-500">de {{ number_format($disponibles) }} disponibles</p>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-4">
            <p class="text-xs text-gray-500">Te quedan</p>
            <p class="text-2xl font-semibold tabular-nums text-gray-900">{{ number_format(max(0, $disponibles - $gastadas)) }}</p>
            <p class="text-xs text-gray-500">hasta el día 1</p>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-4">
            <p class="text-xs text-gray-500">Tu plan</p>
            <p class="pt-1 text-lg font-semibold text-gray-900">{{ $produccion?->plan?->nombre ?? '—' }}</p>
            <p class="text-xs text-gray-500">{{ collect($produccion?->servicios ?? [])->map(fn($s) => strtoupper($s))->join(' y ') }}</p>
        </div>
        <div class="rounded-xl border border-gray-200 bg-white p-4">
            <p class="text-xs text-gray-500">Tus llaves</p>
            <p class="pt-1 text-lg font-semibold text-gray-900">{{ $llaves->count() }}</p>
            <p class="text-xs text-gray-500">{{ $llaves->where('entorno', 'produccion')->count() }} en producción</p>
        </div>
    </div>
    ...
    <div class="grid gap-4 lg:grid-cols-5">
        <div class="rounded-xl border border-gray-200 bg-white lg:col-span-2">
            <div class="flex items-center justify-between border-b border-gray-100 px-4 py-3">
                ...
                <span class="text-xs text-gray-400">Se reinicia el día 1</span>
            </div>
