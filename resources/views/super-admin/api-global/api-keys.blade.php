                        ...
                        <td class="py-3 px-4">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <form method="POST" action="{{ route('super-admin.api-global.regenerate-key', $apiKey) }}"
                                      ...
                                      @submit.prevent="if (confirm('Se genera un Secret nuevo para «' + @js($apiKey->name) + '». El actual deja de funcionar al instante, así que hay que pasarle el nuevo al cliente. La X-Api-Key no cambia. ¿Seguir?')) window.enviarYAbrirModal($el, '{{ route('super-admin.api-global.show-key', $apiKey) }}', 'Detalle de API Key')">
                                    @csrf
