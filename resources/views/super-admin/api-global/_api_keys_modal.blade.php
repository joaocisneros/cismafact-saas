    ...
    <div class="max-h-[65vh] space-y-4 overflow-y-auto">
        @forelse($apiKeys as $apiKey)
            ...
            <x-ficha-credencial :suelta="false">
                ...
                <x-slot:nota>
                    Si el cliente lo perdió, genérale uno nuevo con el botón de renovar de su fila —su X-Api-Key no cambia—.
                </x-slot:nota>

                <x-slot:metricas>
                    <x-metrica-credencial titulo="Llamadas">{{ number_format($consumo[$apiKey->id] ?? 0) }}</x-metrica-credencial>
                    ...
                    <x-metrica-credencial titulo="Vence">{{ $apiKey->expires_at?->format('d/m/Y') ?? 'Nunca' }}</x-metrica-credencial>
                </x-slot:metricas>
