...
<div class="mx-auto max-w-3xl space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-semibold text-gray-900">Crear Empresa</h1>
            <p class="mt-1 text-sm text-gray-500">Formulario liviano para registrar empresa y usuario administrador.</p>
        </div>
        <a href="{{ route('super-admin.companies.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Volver</a>
    </div>

    <form action="{{ route('super-admin.companies.store') }}" method="POST" class="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        @csrf

        <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
            <div>
                <label class="mb-1 block text-sm font-medium text-gray-700">RUC</label>
                <input type="text" name="ruc" value="{{ old('ruc') }}" required maxlength="11" pattern="[0-9]{11}" placeholder="11 digitos"
            ...
            <div>
                ...
                <input type="text" name="direccion" value="{{ old('direccion') }}" placeholder="Direccion fiscal"
                       class="w-full rounded-md border border-gray-300 px-3 py-2 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20">
        ...
        <div class="mt-6 flex justify-end gap-3">
            <a href="{{ route('super-admin.companies.index') }}" class="rounded-md border border-gray-300 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50">Cancelar</a>
