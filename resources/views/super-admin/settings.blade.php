...
@section('content')
{{-- Las cuatro fichas iban una debajo de otra en una sola columna y la
     pantalla pedía tres pantallazos de scroll para algo que se toca una vez
     al mes. En dos columnas cabe de una vez, sin quitar nada ni apretarlo:
     a la izquierda lo que identifica la plataforma, a la derecha lo que la
...
<div class="mx-auto max-w-6xl space-y-5">
    ...
    >
        ...
        <div class="grid gap-5 lg:grid-cols-2 lg:items-start">
            ...
            {{-- Columna izquierda: quién es la plataforma. --}}
            <div class="space-y-5">
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <label class="block">
                            ...
                            <input name="app_name" value="{{ old('app_name', $settings['app_name'] ?? 'Cisma Fact') }}" required maxlength="255"
                                   class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500">
                ...
                <section class="overflow-hidden rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <div class="mt-1.5 flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                                <img src="{{ !empty($settings['platform_logo']) ? Storage::disk('public')->url($settings['platform_logo']) : asset('assets/brand/cisma-fact.png') }}"
                                     alt="Logo actual" class="max-h-12 max-w-32 object-contain">
                                <input id="platform_logo" type="file" name="platform_logo" accept=".png,.jpg,.jpeg,.webp"
                                       class="min-w-0 text-sm text-slate-600 file:mr-2 file:rounded-md file:border-0 file:bg-blue-50 file:px-2.5 file:py-1.5 file:text-blue-700">
                            </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1.5 flex items-center gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                                <img src="{{ !empty($settings['favicon']) ? Storage::disk('public')->url($settings['favicon']) : asset('assets/brand/favicon.png') }}"
                                     alt="Favicon actual" class="h-10 w-10 object-contain">
            ...
            {{-- Columna derecha: cómo se comunica y si está abierta. --}}
            <div class="space-y-5">

                <section class="overflow-hidden rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    <h2 class="text-base font-semibold text-slate-900">Correo SMTP</h2>
                    <p class="mt-0.5 text-sm text-slate-500">Se utilizará para recuperación de contraseñas y notificaciones.</p>
                    ...
                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                        ...
                        <label class="block">
                            ...
                            <select name="smtp_encryption" class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500">
                        ...
                        <label class="block">
                            ...
                            <input type="password" name="smtp_password" maxlength="255" autocomplete="new-password"
                                   placeholder="{{ isset($settings['smtp_password']) ? 'Guardada' : 'Contraseña SMTP' }}"
                                   class="mt-1 w-full rounded-lg border border-slate-300 px-3 py-2 focus:border-blue-500 focus:ring-blue-500">
                            @isset($settings['smtp_password'])
                                <span class="mt-1 block text-xs text-slate-500">Déjala vacía para conservarla.</span>
                            @endisset
                        </label>
                ...
                <section class="overflow-hidden rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div class="mt-4">
                        <select name="maintenance_mode" class="w-full rounded-lg border border-slate-300 px-3 py-2">
                        ...
                        <p class="mt-2 text-sm text-slate-500">
                            Mantenimiento bloquea todo el sitio. Para recuperarlo desde el servidor ejecuta
                            <code class="rounded bg-slate-100 px-1.5 py-0.5">php artisan up</code>.
                        </p>
                    </div>
                </section>
            </div>
