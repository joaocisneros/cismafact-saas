...
use App\Http\Controllers\Web\Consultas\PanelController as ConsultasPanelController;
 ...
 * A cada uno su panel.
 *
 * Tenia dos salidas: el panel de admin, o el de empresa para todos los demas.
 * Cuando apareicieron los clientes de RUC y DNI —que entran al sistema pero no
 * son de ninguna empresa— acababan aqui rebotando: se les mandaba a
 * /empresa/dashboard, el middleware de roles los devolvia a /dashboard, y
 * vuelta a empezar hasta que el navegador cortaba con ERR_TOO_MANY_REDIRECTS.
 * Les pasaba con cualquier direccion que no fuera suya.
 *
 * Por eso ya nadie cae en el panel de empresa por descarte: se entra ahi si se
 * es de una empresa, y a quien no encaje en ningun panel se le dice, en vez de
 * mandarlo a una puerta que va a devolverlo aqui.
 */
Route::middleware('auth', 'company.active')->get('/dashboard', function () {
    ...
    return redirect()->route('login')->withErrors([
        'email' => 'Tu cuenta no tiene un panel asignado. Avisa al administrador.',
    ]);
})->name('dashboard');
...
Route::prefix('super-admin')
    ...
    ->group(function () {
        ...
        // Quien entra al panel a ver su llave. El acceso cuelga del titular:
        // con dos llaves de produccion entra una vez y ve las dos.
        Route::post('/consultas/acceso', [\App\Http\Controllers\Web\SuperAdmin\AccesoConsultasController::class, 'crear'])
            ->name('consultas.acceso.crear');
...
/*
|--------------------------------------------------------------------------
| El panel del cliente de RUC y DNI
|--------------------------------------------------------------------------
|
| Quien solo compra consultas no es una empresa: no factura, no tiene RUC
| emisor ni certificado, asi que el panel de empresa no le sirve. Aqui ve su
| llave, su consumo y sus consultas, y nada mas del sistema.
|
| Entra por el mismo formulario de login que todos; lo que cambia es a donde
| le manda despues, segun su rol.
|
*/
Route::prefix('consultas')
    ->middleware(['auth', 'role:cliente_consultas'])
    ->name('consultas.')
    ->group(function () {
        Route::get('/', [ConsultasPanelController::class, 'panel'])->name('panel');
        Route::get('/mi-api', [ConsultasPanelController::class, 'credenciales'])->name('credenciales');
        ...
        Route::put('/mi-cuenta/clave', [ConsultasPanelController::class, 'cambiarClave'])->name('cuenta.clave');

        // Solo el secreto: la clave identifica la llave y sale en los
        // registros, cambiarla obligaria a rehacer la integracion entera.
        Route::post('/mi-api/{llave}/secreto', [ConsultasPanelController::class, 'regenerarSecreto'])
            ->name('secreto.regenerar');
    });
